...
for (experiment,etag) in experiments_to_process:
    with open(pipe_path + 'bronze/entity/' + experiment + '.json', 'r') as file:
    ...
    for attribute in schema_carcass['entity']:
    ...
    sparkDF = sparkDF.select(
        ...
        'src_method' ,
        'type'  )
    sparkDF.write.insertInto('pdb_pipeline.bronze_entity', overwrite=False)

    experiments_for_sql = f"{experiments_for_sql}'{experiment}',"

experiments_for_sql = experiments_for_sql[:len(experiments_for_sql)-1]
for attribute in schema_carcass['entity']:
    stmnt = f"update pdb_pipeline.bronze_entity set {attribute} = Null where (experiment in ({experiments_for_sql}) and {attribute} = '?')"
    #print(stmnt)
    spark.sql(stmnt)
